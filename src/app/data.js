...
const baseState = parsedData.boards.reduce((acc, cv) => {
  acc.boards = acc.boards || {};
  acc.boards.byId = acc.boards.byId || {};
  acc.boards.allIds = acc.boards.allIds || [];
  acc.boards.currentlySelected = acc.boards.currentlySelected || "";
  acc.columns = acc.columns || {};
  acc.columns.byId = acc.columns.byId || {};
  acc.columns.allIds = acc.columns.allIds || [];
  acc.tasks = acc.tasks || {};
  acc.tasks.byId = acc.tasks.byId || {};
  acc.tasks.allIds = acc.tasks.allIds || [];

  const boardId = uuidv4();

  acc.boards.allIds.push(boardId);

  const columns = cv.columns.map((el) => {
    return {
      columnId: uuidv4(),
      name: el.name,
      tasks: el.tasks,
  ...
  acc.boards.byId[boardId] = {
    boardId,
    name: cv.name,
    columnIds: columns.map((el) => el.columnId),
  };

  acc.columns.allIds.push(...columns.map((el) => el.columnId));
...
baseState.boards.currentlySelected = baseState.boards.allIds[0];

export default JSON.stringify(baseState);